...
function pageBanner($args = []) {
    // Set default values for title, subtitle, and photo
    if (!isset($args['title'])) {
...
function university_features(){
    ...
    add_image_size('professorProtrait',480,660,true);
    add_image_size('pagebanner',1500,350,true);
}
add_action('after_setup_theme','university_features');
// to create a theme 
function university_adjust_queries($query){
    // by is admin here described that the post type is not connected to the backend 
    if(!is_admin()  AND is_post_type_archive('program') AND $query->is_main_query())
{
    $query->set('orderby','title' );
    $query->set( 'order','ASC');
     $query->set( 'post_per_page',-1);
    //  it means as much post as it can show max per page
}    
...
function redirectSubsToFrontend() {
    ...
    if (count($ourCurrentUser->roles) == 1 && $ourCurrentUser->roles[0] == 'subscriber') { // Corrected roles check
